...
func (h *Handler) Router() chi.Router {
	...
	// Use case 6: medals of a country per discipline (Postgres).
	r.Get("/countries/{countryID}/games/{gameID}/medals-by-discipline", h.medalsByCountryAndDiscipline)

	// Use case 7: athletes with >N medals OR an olympic record (Redis + Mongo).
	r.Get("/games/{gameID}/top-athletes", h.topAthletes)
...
func (h *Handler) medalsByCountryAndDiscipline(w http.ResponseWriter, r *http.Request) {
	...
	out, err := h.svc.MedalsByCountryAndDiscipline(r.Context(), countryID, gameID)
	respond(w, out, err)
}
...
func (h *Handler) topAthletes(w http.ResponseWriter, r *http.Request) {
	...
	out, err := h.svc.TopAthletes(r.Context(), gameID, queryInt(r, "min", 4))
	respond(w, out, err)
}
