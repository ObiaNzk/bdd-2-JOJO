...
func (c *Console) medalsByCountryDiscipline(ctx context.Context) {
	c.explain("Neo4j", `MATCH (c:Country {id:$countryId})<-[:REPRESENTS]-(t:Team)-[:IN_GAME]->(:OlympicGame {id:$gameId})
MATCH (t)-[:WON]->(m:Medal)
MATCH (t)-[:COMPETED_IN]->(:Event)-[:OF]->(d:Discipline)
RETURN d.id, d.name,
...
ORDER BY total DESC`)
	...
	rows, err := c.svc.MedalsByCountryAndDiscipline(ctx, countryID, gameID)
	if err != nil {
		c.fail(err)
		return
	}
...
func (c *Console) topAthletes(ctx context.Context) {
	c.explain("Redis + MongoDB", `Redis:   ZRANGEBYSCORE medals:athlete:{gameID} {min} +inf WITHSCORES   -- atletas con >= N medallas
MongoDB: db.event_results.aggregate([{ $unwind: "$records" },
           { $project: { athleteName: "$records.athleteName" } }])              -- quién tiene récord
         (se combinan: aparece quien cumple medallas O tiene récord)`)
	c.printGames(ctx)
	gameID, ok := c.askInt("ID juego: ")
	if !ok {
		return
	...
	rows, err := c.svc.TopAthletes(ctx, gameID, int(min))
	if err != nil {
		c.fail(err)
		return
	}
