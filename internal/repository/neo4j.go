...
// medals in a game, broken down by discipline.
func (r *Neo4jRepository) CountMedalsByCountryAndDiscipline(ctx context.Context, countryID, gameID int64) ([]model.DisciplineMedalCount, error) {
	res, err := neo4j.ExecuteQuery(ctx, r.driver,
		`MATCH (c:Country {id: $countryId})<-[:REPRESENTS]-(t:Team)-[:IN_GAME]->(:OlympicGame {id: $gameId})
		 MATCH (t)-[:WON]->(m:Medal)
		 ...
		 RETURN d.id AS disciplineId, d.name AS disciplineName,
		        ...
		        count(m) AS total
		 ORDER BY total DESC`,
		map[string]any{"countryId": countryID, "gameId": gameID},
		neo4j.EagerResultTransformer, neo4j.ExecuteQueryWithDatabase("neo4j"))
	if err != nil {
		return nil, err
	}
